      ...
      <div class="flex flex-col md:odd:flex-row md:even:flex-row-reverse h-auto mb-8 md:mb-12 lg:mb-16 2xl:mb-32 gap-4 md:gap-8 lg:gap-12 2xl:gap-16 rounded-b-md rounded-t-md shadow-md md:shadow-none">
        <div class="w-full md:w-1/2 relative">
          <div class="overlay md:flex justify-center">
            <div class="w-full h-full bg-title md:hover:bg-transparent md:hover:text-transparent bg-opacity-80 py-32 md:py-0 px-4 absolute top-0 left-0 flex justify-center items-center rounded-t-md md:rounded-md text-2xl md:text-3xl lg:text-5xl font-bold text-white">
              <span class="text-center">${blog.title}</span>
            </div>
            <img src="${blog.image}" alt="${blog.title}" class="w-full h-72 rounded-md" />
          </div>
          <div class="hidden md:block w-full absolute bottom-0 left-0 text-lg text-center text-white py-2 rounded-b-md bg-greenDark z-30">Voir Plus</div>
        </div>
        <div class="w-full md:w-1/2 md:py-0 relative">
          <p class="px-2 text-lg md:text-xl lg:text-2xl mb-2">
            ${blog.text}
          </p>
          <div class="w-full md:hidden text-lg text-center text-white py-2 rounded-b-md bg-greenDark select-none">Voir Plus</div>
        </div>
      </div>
    `;
  });
